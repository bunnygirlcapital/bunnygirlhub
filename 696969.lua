...
do
    ...
    local SelectEggs = Section:AddDropdown("SelectEggs", {
        Title = "Select Eggs",
        Values = {"BoneDragonEgg", "UltraEgg", "UnicornProEgg", "SaberCubEgg", "GeneralKongEgg", "GodzillaEgg", "MetroGiraffeEgg", "BiteForceSharkEgg"},
        Multi = true,
        ...
        local eggConnection = islandFolder.ChildAdded:Connect(function(eggObject)
            ...
            if uid or eggObject.Name:match("^[a-f0-9]+$") then
                ...
                if Options.AutoBuyEgg.Value then
                    ...
                    for egg, state in next, Options.SelectEggs.Value do
                        if state then table.insert(selectedEggs, egg) end
                    end
                    
                    for mutation, state in next, Options.SelectMutations.Value do
                        if state then table.insert(selectedMutations, mutation) end
                    end
...
do
    ...
    if loadFoodConfig() then
        ...
        if loadFoodConfig() then
            ...
            foodNames = {"Strawberry", "Blueberry", "Watermelon", "Apple", "Orange", "Corn", "Banana", "Grape", "Pear", "Pineapple", "DragonFruit", "GoldMango", "BloodstoneCycad", "ColossalPinecone", "VoltGinkgo", "DeepseaPearlFruit", "CandyCorn", "Durian", "Pumpkin", "FrankenKiwi"}
    ...
    local SelectFood = FoodSection:AddDropdown("SelectFood", {
        ...
        Values = foodNames,
        Multi = true,
        Default = {"Pear", "Pineapple", "DragonFruit", "GoldMango", "BloodstoneCycad", "ColossalPinecone", "VoltGinkgo", "DeepseaPearlFruit", "CandyCorn", "Durian", "Pumpkin", "FrankenKiwi"} -- SaveManager will restore saved selections automatically
    })
...
do
    ...
    local function formatNumber(num)
        if not Format then return tostring(num) end
    ...
    local function handleTrade(tradeData)
        ...
        local playerValue = getPetValue(playerPet)
        print("💰 [Auto Trade] Your pet value: " .. formatNumber(playerValue) .. " (Type: " .. tostring(playerPet.T) .. ", Mutation: " .. tostring(playerPet.M) .. ")")
        
        local traderValue = 0
        for i, pet in pairs(traderPets) do
            local petValue = getPetValue(pet)
            print("💰 [Auto Trade] Trader pet #" .. i .. " value: " .. formatNumber(petValue) .. " (Type: " .. tostring(pet.T) .. ", Mutation: " .. tostring(pet.M) .. ")")
    ...
    TradeZoneDropdown:OnChanged(function(value)
        ...
        if zoneLevel and ReplicatedStorage then
            local TradeZoneRE = ReplicatedStorage:FindFirstChild("Remote") and 
...
do
    ...
    WebhookSection:AddButton({
        ...
        Callback = function()
            ...
            local message = string.format("**Egg Inventory Report**\nTotal Eggs: %d\n\n%s", eggCount, table.concat(eggDetails, "\n"))
...
print("🔍 [Config Load] MinFairnessPercentage loaded as:", Options.MinFairnessPercentage and string.format("%.2f", Options.MinFairnessPercentage.Value) or "nil")
